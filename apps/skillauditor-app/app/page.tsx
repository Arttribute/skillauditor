...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col min-h-full bg-white">
      ...
      {/* 3px blue accent line at the very top */}
      ...
      <main className="flex flex-col flex-1">
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="flex flex-col gap-8">
              <div className="flex flex-col gap-5">
                ...
                <span className="inline-flex self-start items-center gap-2 rounded-full bg-[#0052ff] px-3 py-1 text-xs font-semibold text-white">
                ...
                <p className="text-base text-zinc-500 leading-relaxed max-w-md">
                  A skill can quietly expand an agent&apos;s scope, extract secrets, or hide behavior.
                  SkillAuditor audits and verifies every skill before it runs.
                </p>
              </div>
            ...
            <div className="rounded-xl border border-zinc-200 overflow-hidden shadow-sm">
              ...
              <div className="px-5 py-3 bg-zinc-900 flex items-center justify-between">
                <span className="text-xs font-semibold text-zinc-400 uppercase tracking-widest">Threat vectors</span>
                <span className="text-xs text-zinc-600">detected by SkillAuditor</span>
              </div>
              <div className="flex flex-col divide-y divide-zinc-100">
                {[
                  { label: 'Scope manipulation',     color: 'bg-orange-500', light: 'bg-orange-50',  text: 'text-orange-600', desc: 'Instructions that push agents beyond declared permissions' },
                  { label: 'Secret exfiltration',    color: 'bg-yellow-400', light: 'bg-yellow-50',  text: 'text-yellow-700', desc: 'Directives routing credentials to unauthorized endpoints' },
                  { label: 'Behavioral deception',   color: 'bg-[#0052ff]',  light: 'bg-[#eff4ff]', text: 'text-[#0052ff]',  desc: 'Skills that behave differently under analysis than in execution' },
                  { label: 'Downstream manipulation',color: 'bg-zinc-400',   light: 'bg-zinc-50',   text: 'text-zinc-500',   desc: 'Subverting actions and outputs visible to agents and users' },
                ].map(({ label, color, light, text, desc }) => (
                  <div key={label} className="flex items-center gap-4 px-5 py-4 bg-white hover:bg-zinc-50/60 transition-colors">
                    <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${light}`}>
        ...
        <section className="border-b border-zinc-100 relative overflow-hidden bg-[#f7faff]">
          ...
          <div className="relative max-w-5xl mx-auto px-8 py-20">
            <div className="mb-14">
              <p className="text-xs font-semibold tracking-widest text-[#0052ff] uppercase mb-3">How it works</p>
            ...
            <div className="flex flex-col">
              {[
                { num: '01', solidBg: 'bg-orange-500',  title: 'Structural Extraction', desc: 'SHA-256 content hash, frontmatter parse, declared capabilities, external URLs. Deterministic and tamper-evident.' },
                { num: '02', solidBg: 'bg-yellow-400',  title: 'Content Analysis',       desc: 'An LLM examines every instruction for deception patterns, injection attempts, and exfiltration directives.' },
                { num: '03', solidBg: 'bg-[#0052ff]',   title: 'Sandbox Simulation',     desc: 'The skill runs in an isolated environment with honeypot credentials and 14 intercepted tool types. What it says vs. what it does.' },
                { num: '04', solidBg: 'bg-zinc-800',    title: 'Verdict Synthesis',       desc: 'A judging layer reconciles all findings into a scored verdict and tamper-proof onchain record on Base.' },
              ].map(({ num, solidBg, title, desc }, i) => (
                <div key={num} className={`grid grid-cols-[64px_1fr] gap-6 py-8 ${i < 3 ? 'border-b border-zinc-200/60' : ''}`}>
                  <div className={`flex items-center justify-center h-10 w-10 rounded-xl text-sm font-bold text-white shadow-sm ${solidBg}`}>
                    {num}
                  </div>
        ...
        <section className="border-b border-zinc-100 px-8 py-20">
          <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-14 items-center">
            <div className="flex flex-col gap-6">
              <div>
                <p className="text-xs font-semibold tracking-widest text-[#0052ff] uppercase mb-3">For developers</p>
              ...
              <div className="flex flex-col gap-2.5 text-sm text-zinc-600">
                <div className="flex items-center gap-3">
                  <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-orange-50"><span className="h-2 w-2 rounded-full bg-orange-500" /></span>
                  REST API &amp; CLI
                </div>
                <div className="flex items-center gap-3">
                  <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-yellow-50"><span className="h-2 w-2 rounded-full bg-yellow-400" /></span>
                  Machine-readable JSON verdicts
                </div>
                <div className="flex items-center gap-3">
                  <span className="flex h-6 w-6 items-center justify-center rounded-lg bg-[#eff4ff]"><span className="h-2 w-2 rounded-full bg-[#0052ff]" /></span>
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-950 overflow-hidden text-xs font-mono leading-relaxed shadow-xl shadow-zinc-900/20">
              ...
              <div className="px-5 py-5 flex flex-col gap-0.5">
                <p className="text-zinc-600">{'# audit a skill before loading it into your agent'}</p>
                <p className="mt-2">
                  <span className="text-zinc-500">$ </span>
                  <span className="text-zinc-300">npx @skillauditor/client verify ./SKILL.md</span>
                </p>
                <p className="mt-3">
                  <span className="text-[#0052ff]">▶  </span>
                  <span className="text-zinc-300">Auditing: GitHub PR Reviewer  </span>
                  <span className="text-zinc-600">[audit-a1b2c3d4]</span>
                </p>
                <p className="mt-2 pl-3"><span className="text-emerald-400">✔  </span><span className="text-zinc-500">Stage 1 — Structural extraction complete</span></p>
                <p className="pl-3"><span className="text-emerald-400">✔  </span><span className="text-zinc-500">Stage 2 — Content analysis complete</span></p>
                <p className="pl-3"><span className="text-emerald-400">✔  </span><span className="text-zinc-500">Stage 3 — Sandbox simulation complete</span></p>
                <p className="pl-3"><span className="text-emerald-400">✔  </span><span className="text-zinc-500">Stage 4 — Verdict synthesis complete</span></p>
                <p className="mt-3">
                  <span className="text-emerald-400">✔  SAFE  </span>
                  <span className="text-zinc-600">score=</span>
                  <span className="text-emerald-400">91/100</span>
                </p>
                <p className="mt-4 text-zinc-600">{'# use exit code in CI — 0 = safe, 1 = unsafe'}</p>
                <p className="mt-1">
                  <span className="text-zinc-500">$ </span>
                  <span className="text-zinc-300">skillauditor verify ./SKILL.md --silent </span>
                  <span className="text-zinc-500">&amp;&amp; </span>
